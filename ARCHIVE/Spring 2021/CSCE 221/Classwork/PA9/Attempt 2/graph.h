...
class Graph {
 // TODO(student): implement Graph
    ...
    std::list<std::pair<size_t,size_t>> prim(){
        ...
        visited.push_back(aSource);
        size_t repeat = 0;

        int fb = 3; //force break
        while(repeat != visited.size()) {
            repeat = visited.size();
    ...
    double distance(size_t id) const{
        if(dja){
            ...
            if(id == aSource){ return 0; }
            double dist = 0;
            size_t cursor = id;
                std::list<std::pair<size_t, size_t>>::const_iterator it = mstD.begin();
                for(size_t i = 0; i < mstD.size(); ++i) {//finds spot in mst that corresponds to cursor
                    if(it->second != cursor){
                        std::advance(it, 1);
                    }
                    else{ break; }
                }
                dist = cost(it->first, it->second);//updates with cost
